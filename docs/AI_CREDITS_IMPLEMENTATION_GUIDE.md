# AI Credits System Implementation Guide (GeoRise)

This document provides step-by-step prompts to implement the complete AI credit/token system for GeoRise. Copy each prompt in sequence to build the system incrementally.
...
- Lovable Cloud connected (Supabase backend)
- Authentication implemented (users can sign in)
- A `profiles` table with `id` and `plan` columns (free/pro/giftedPro/business/giftedAgency)
- A `user_roles` table with admin role management via `has_role(user_id, role)` function

---

## GeoRise-Specific Adaptations

This guide is adapted from the Querino project with these differences:

| Querino | GeoRise |
|---------|---------|
| `profiles.plan_type` (free/premium) | `profiles.plan` (free/pro/giftedPro/business/giftedAgency) |
| `profiles.role` column | `user_roles` table with `has_role()` function |
| `is_admin()` function to create | Existing `has_role(user_id, 'admin')` function |

**Plan Mapping for Credits:**
- `free` → Free tier credits
- `pro`, `giftedPro` → Pro tier credits  
- `business`, `giftedAgency` → Business tier credits
...
1. `ai_credit_settings` table:
   ...
   - credits_pro_per_month: 1500 (Monthly credits for pro/giftedPro users)
   - credits_business_per_month: 5000 (Monthly credits for business/giftedAgency users)

   RLS: Anyone can SELECT, only admins can UPDATE (use existing has_role function).
...
3. `llm_usage_events` table (append-only ledger):
   ...
   - feature (text, nullable - e.g., 'chat_coach', 'ai_insights', 'admin_balance_adjustment')
   - model (text, nullable)
...
Use the existing has_role(user_id, 'admin') function for admin checks in RLS policies.
```
...
2. Authorization:
   - If user_id differs from caller, require caller to be admin (use has_role function)
...
3. Logic for single user:
   ...
   - If no, create a new period:
     ...
     b. Look up user's plan from profiles table
     c. Determine credits based on plan:
        - 'free' → credits_free_per_month
        - 'pro', 'giftedPro' → credits_pro_per_month
        - 'business', 'giftedAgency' → credits_business_per_month
     d. Get settings from ai_credit_settings
     e. Calculate base_tokens = credits * tokens_per_credit
     f. Check for previous expired period and calculate rollover:
        - rollover_tokens = MIN(previous remaining_tokens, base_tokens)
     g. Insert new period with tokens_granted = base_tokens + rollover_tokens
     h. Store metadata: { base_tokens, rollover_tokens }
...
2. On mount (if user is logged in):
   ...
   - Also fetch user's plan from profiles
...
```
Create a component called `CreditsDisplay` in src/components/account/CreditsDisplay.tsx that:
...
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    url := 'https://jubqccofwpmflhsxtiry.supabase.co/functions/v1/ensure-token-allowance',
...
```
For each AI feature in the app (currently: Chat Coach in CoachGEOvanniCard):
...
```
In the chat-coach edge function (and any future LLM edge functions), after the API call succeeds:

1. Insert a record into llm_usage_events:
   - user_id: from auth
   - idempotency_key: unique key (e.g., `${feature}_${userId}_${timestamp}`)
   - feature: name of the feature (e.g., 'chat_coach')
   - model: the LLM model used (e.g., 'google/gemini-2.5-flash')
...
```
Update the database-schema.sql file to include documentation for:
...
│  CreditsDisplay        - Account page UI with progress bar      │
...
│  chat-coach              - Log usage to llm_usage_events        │
│  [future-ai-features]    - Log usage to llm_usage_events        │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
6. **Plan-based gating** - AI features require Pro/Business plan AND remaining credits
...
|------|---------------|----------|
| `free` | 0 | No AI coach access |
| `pro`, `giftedPro` | 1500 | Chat Coach (50 msg/day rate limit + credit check) |
| `business`, `giftedAgency` | 5000 | Chat Coach (unlimited rate) |

---

## Customization Points

- **Plan tiers**: Adjust `credits_free_per_month`, `credits_pro_per_month`, `credits_business_per_month`
...
## Current AI Features to Integrate

1. **Chat Coach** (`supabase/functions/chat-coach/index.ts`)
   - Currently has message rate limiting (50/day for pro)
   - Needs token tracking integration
